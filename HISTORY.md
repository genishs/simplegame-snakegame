...
이동 과일이 때때로 먹히지 않는 현상이 라이브에서 관측됐다. 원인은 머리(매 tick 1칸)와 과일(tick×4마다 1칸, 4배 느림)이 같은 축에서 서로를 향해 움직일 때 **칸을 맞교환(swap)** 하며 서로 통과하는 것 — 어느 tick 경계에서도 같은 칸을 공유하지 않아 `tick()`의 "머리 칸 == 과일 칸" 먹기 판정이 둘을 놓친다. 통과한 과일은 머리 뒤로 빠지고, 머리가 더 빨라 느린 과일이 영영 따라잡지 못해 먹을 수 없는 상태가 된다.

### What

- **pass-through 먹기 판정 추가**: `tick()`에서 1차로 머리의 새 칸에 과일이 있는지 보고, 없으면 2차로 **머리가 방금 떠난 칸(prevHead)** 에 과일이 있는지 본다. swap·측면 진입으로 과일이 이번 tick에 머리의 경로와 만난 경우를 포착해 먹는다. `eatIdx`를 `const` → `let`으로 변경, **tick당 최대 1개** 불변 유지(머리 칸 우선).
- **불변식 보존**: 먹기는 여전히 tick() 단일 경로(planner 위험 #3). 과일 드리프트는 위치만 바꾸고 먹기/성장/점수를 처리하지 않음 — v0.5.8 prevSnake 렌더 보간 불변식 유지. 비-이동 스테이지는 무해(spawnFood가 점유 칸에 과일을 놓지 않아 정지 과일이 prevHead에 있을 수 없음).
- **검증**: 노드 시뮬레이션으로 swap 마이크로스텝 재현 — 수정 전 `eaten=0`(이탈), 수정 후 `eaten=1`.

### 범위 밖 (보류)

- St3 `fruitMoves:true` 잠정 테스트 플래그 **유지**. 수정 버전을 라이브에서 재체감한 뒤 정식화(St7-only) 여부를 별도 결정. 이동 과일 속도(tick×4) 튜닝도 재테스트 후 판단.
